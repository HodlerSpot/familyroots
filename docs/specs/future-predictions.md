...
| Author account erasure (GDPR) | Attribution anonymized to "A family member"; text is retained by default as family-vault content, but the erasure runbook honors a specific request to delete the text too — including from **sealed** rounds via the admin path (the one sanctioned read/write into sealed data). Sealed images are **re-rendered** if an erased word materially came only from the erased prediction (admin reconcile step in the runbook). |
| Child profile deleted | Cascade: rounds, predictions, and cloud media for that child are hard-deleted, sealed or not (child erasure beats the time capsule — same rule as the rest of the vault). |
| Family deleted | Same cascade for every child in the family. |
| Duplicate submit race (double-tap) | Unique (`round_id`, `author_user_id`) makes the second insert a no-op/friendly conflict; exactly one `prediction_added` event. |
