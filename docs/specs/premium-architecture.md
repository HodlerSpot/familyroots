...
> **✅ RESOLVED (2026-07) — leave/remove endpoints wired.** `POST /families/{id}/members/me/leave` and `DELETE /families/{id}/members/{user_id}` (in `routers/families.py`) mark the membership `removed` and call `handle_owner_departure` in the same transaction, so a departing subscription owner stops auto-renewal at period end and the remaining parents are emailed (tests: `tests/test_membership.py`). Account deletion still does not exist; when it lands it must call this hook too.
