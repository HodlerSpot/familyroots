...
class MediaStorage(Protocol):
    ...
    def put_object(self, storage_key: str, data: bytes, content_type: str) -> int:
        """Server-side write of generated bytes (e.g. the sealed cloud PNG).
        Returns the byte size. Content type is stored so the download path
        serves it back correctly (browsers need image/png in an <img>)."""
        ...
...
class S3MediaStorage:
    ...
    def put_object(self, storage_key: str, data: bytes, content_type: str) -> int:
        # Server-side write (s3:PutObject is already granted). Unlike client
        # uploads, generated media bytes flow through the API for this one path.
        self.client.put_object(
            Bucket=self.bucket, Key=storage_key, Body=data, ContentType=content_type
        )
        return len(data)
